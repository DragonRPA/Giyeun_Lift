...
/**
 * 인접 자음 전치(Swap) 또는 1자 오타를 허용하는 초성 퍼지 매칭
 * 예: 'ㅅㅂㅇㅇ' -> 'ㅂㅅㅇㅇ' 매칭 ('백산이엔씨')
 * 
 * @param target 대상 문자열 (예: '백산이엔씨')
 * @param query 검색어 (예: 'ㅅㅂㅇㅇ')
 * @returns 일치 여부
 */
export function matchHangulFuzzy(target?: string | null, query?: string | null): boolean {
  if (!query || !query.trim()) return true;
  if (!target || !target.trim()) return false;
  if (matchHangul(target, query)) return true;

  const cleanTarget = target.replace(/주식회사|\(주\)|\s/g, '').trim();
  const cleanQuery = query.replace(/\s/g, '').trim();
  const targetChosung = extractChosung(cleanTarget);

  // 1. 인접 자음 전치(Transposition/Swap) 검사 (예: 'ㅅㅂㅇㅇ' <-> 'ㅂㅅㅇㅇ')
  if (cleanQuery.length >= 2) {
    for (let i = 0; i < cleanQuery.length - 1; i++) {
      const swapped = cleanQuery.slice(0, i) + cleanQuery[i + 1] + cleanQuery[i] + cleanQuery.slice(i + 2);
      if (targetChosung.includes(swapped) || matchHangul(cleanTarget, swapped)) {
        return true;
      }
    }
  }

  // 2. 3글자 이상인 경우 1자 오타(1-character deletion) 검사
  if (cleanQuery.length >= 3) {
    for (let i = 0; i < cleanQuery.length; i++) {
      const sub = cleanQuery.slice(0, i) + cleanQuery.slice(i + 1);
      if (sub.length >= 2 && (targetChosung.includes(sub) || matchHangul(cleanTarget, sub))) {
        return true;
      }
    }
  }
...
/**
 * 복수의 대상 문자열 중 하나라도 초성 퍼지 검색어와 일치하는지 검사
 */
export function matchHangulFuzzyAny(targets: (string | null | undefined)[], query?: string | null): boolean {
  if (!query || !query.trim()) return true;
  return targets.some(target => matchHangulFuzzy(target, query));
}
