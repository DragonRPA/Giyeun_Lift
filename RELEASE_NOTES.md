...
### 🎙️ [PC/모바일 공통] 음성 STT 인식 결과 시각화 배지(원클릭 키보드 수정) & 한글 초성·자음 도치 오타 퍼지 검색 엔진 구축 & WTT 44회 관통 검증 완결
- **헌장 1.1 최우선 편익 / 헌장 1.2 ③ 임직원 업무 최소 조작 / 헌장 3.1 무수식어 전문 표준 / 헌장 5.5 WTT 검증**:
  ...
  - **한글 초성(Chosung) 및 자음 전치(Swap) 오타 퍼지 검색 엔진 전면 연동 (`hangulSearch.ts`)**:
    - `matchHangulFuzzy`, `matchHangulFuzzyAny` 신설: 인접 자음 순서가 바뀐 타이핑 오타(`ㅅㅂㅇㅇ` <-> `ㅂㅅㅇㅇ`) 및 1글자 오타 편집거리를 허용하여 유사 거래처 자동 감지.
  - **파서 계층 초성 계층적 우선순위 랭킹 체계 도입 (`voiceOrderDraftService.ts`)**:
    - `parseCustomerVoiceInput`: ①완전일치 ➔ ②완성형접두 ➔ ③초성완전일치 ➔ ④초성접두일치(ALLOWED 우선) ➔ ⑤완성형포함 ➔ ⑥초성부분일치 ➔ ⑦대표자 ➔ ⑧초성퍼지매칭의 8단계 정밀 랭킹 도입.
    - `ㅂㅅ` 또는 `ㅂㅅㅇㅇ` 입력 시 `백산이엔씨` 자동 선택.
    - `ㅅㅇ` 입력 시 `세연테크` 자동 선택.
    - `ㅅㅂㅇㅇ` 입력 시 자음 도치 보정으로 `백산이엔씨` 퍼지 매칭.
  ...
  - **도메인 관통 스트레스 테스트 WTT 44회 전수 통과 (`src/tests/wtt_voice_dispatch.test.ts`)**:
    ...
    - `WTT-DISP-42`: 초성 자음 전치(Swap) 오타 퍼지 보정 검증 (ㅅㅂㅇㅇ -> 백산이엔씨) ➔ **PASS**.
